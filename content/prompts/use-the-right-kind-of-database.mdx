...
Even within each database paradigm **there are decisions to be made** around which product to use. In the case of Key-Value databases for example, there are key differences between [Redis](https://en.wikipedia.org/wiki/Redis) and [memcached](https://en.wikipedia.org/wiki/Memcached);
