...
For many years, SQL based databases such as Postgres, SQL Server and MySQL have dominated the online database conversation. As the web has matured, certain patterns emerged and it became apparent that SQL was not suitable for all use cases. Eventually these patterns became common enough to spawn database architectures specifically built around those use cases.
