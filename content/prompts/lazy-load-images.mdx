...
This change ensures the request for `foobar.jpg` is only sent if the user scrolls to the image's location. This technique is supported by screen readers, and also works for `iframe` tags.
