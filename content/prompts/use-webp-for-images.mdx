...
Note that a successor to this format already exists in the form of AVIF, however [AVIF browser support](https://caniuse.com/avif) is not yet mature enough to use it without a fallback image (see Compatibility below).
...
This will detect whether or not browser support exists for WebP and then use that in place of the jpeg if the format is supported (the jpeg, in this instance, would not be downloaded at all). The `picture` element also has [near universal browser support](https://caniuse.com/picture).
